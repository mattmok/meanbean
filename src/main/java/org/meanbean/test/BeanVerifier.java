...
public interface BeanVerifier {
    ...
    public static BeanVerifiers beans(Class<?>... beanClasses) {
        BeanVerifiers beanVerifiers = new BeanVerifiers();
        for (Class<?> beanClass : beanClasses) {
            try {
                beanVerifiers.addVerifier(BeanVerifier.forClass(beanClass));
            } catch (AssertionError | RuntimeException e) {
                throw new AssertionError("Cannot verify bean type " + beanClass.getName(), e);
            }
        }
        return beanVerifiers;
    }
    ...
    /**
     * Start a fluent bean verification chain for the given packageName
     *
     * @param packageName
     * @return BeanVerifiers
     */
    static BeanVerifiers beansIn(String packageName) {
        Class<?>[] beanClasses = ClassPathUtils.findClassesIn(packageName);
        return beans(beanClasses);
    }
     ...
     * @return BeanVerifiers
     */
    static BeanVerifiers beansIn(String packageName, Class<?>... excludes) {
        Class<?>[] beanClasses = ClassPathUtils.findClassesIn(packageName, excludes);
        return beans(beanClasses);
    }

    /**
     * tart a fluent bean verification chain for the given packageObj
     *
     * @param packageObj
     * @return BeanVerifiers
     */
    static BeanVerifiers beansIn(Package packageObj) {
        return beansIn(packageObj.getName());
    }
     ...
     * @return BeanVerifiers
     */
    static BeanVerifiers beansIn(Package packageObj, Class<?>... excludes) {
        return beansIn(packageObj.getName(), excludes);
    }
     ...
     */
    default void verify() {
        verifyGettersAndSetters().verifyToString();
    }
